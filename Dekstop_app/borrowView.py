...
"border-left: 1px solid grey;")
    ...
    def get_list(self):
        ...
        try:
            resp = requests.post('http://192.168.0.107:5000/api/getborrow')
            resp_2 = requests.post('http://192.168.0.107:5000/api/getreader')
            resp_3 = requests.post('http://192.168.0.107:5000/api/getbook')
        except:
        ...
        for key, value in self.data.items():
            self.itemModel.setItem(int(key), 0, QtGui.QStandardItem(self.resp[str(value[1])][1]))
            self.itemModel.setItem(int(key), 1, QtGui.QStandardItem(self.resp_2[str(value[2])][1]))
    ...
    def search(self):
        ...
        i = 0
        for key, value in self.data.items():
            if cache.upper() in self.resp[str(value[1])][1].upper() or cache.upper() in self.resp_2[str(value[2])][1].upper() or cache.upper() in value[0].upper() or cache.upper() in '':
