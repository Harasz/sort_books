...
"background-color: grey;")
    ...
    def add_re(self):
        ...
        try:
            resp = requests.post(self.Config.get_server()+'/api/addre', data={'key': open('.cache', 'r').read(), 'arg1': name, 'arg2': address, 'login': login})
            check_con(resp)
        except:
        ...
        if 507 == resp.status_code:
            ...
            self.clear_all()
            self.label_4.setStyleSheet("color: green;")
            self.show_userInfo(login, json.loads(resp.text)['haslo'])
