...
"background-color: grey;")
    ...
    def get_list_book(self):
        try:
            resp = requests.post(self.Config.get_server()+'/api/getbook', data={'key': open('.cache', 'r').read()})
            check_con(resp)
        except:
    ...
    def add_borrow(self):
        ...
        try:
            resp = requests.post(self.Config.get_server()+'/api/borrow', data={'key': open('.cache', 'r').read(), 'arg1': date_2, 'arg2': date_1, 'name_id': self.list_1[reader], 'book_id': self.list_2[book]})
            check_con(resp)
        except:
