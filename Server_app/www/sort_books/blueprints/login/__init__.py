...
def auth_in():
    ...
    try:
        if request.method == 'POST' and not 'auth' in session:
            ...
            if resp:
                session['auth'] = {'id': resp[0], 'username': resp[1], 'address': resp[2], 'login': resp[6]}
                if not resp[5]:
            ...
            if 'email' in request.form:
                ...
                if request.form['pass'] == request.form['pass2']:
                    cur.execute("UPDATE librarians.readers SET pass=%s WHERE id_r=%s;",
                                (sha512(request.form['pass'].encode('UTF-8')).hexdigest(), session['auth']['id']))
                ...
                cur.execute("INSERT INTO librarians.readers_pref VALUES (%s, %s, %s, %s);",
                            (session['auth']['id'], allow_email, allow_address, allow_profile))
