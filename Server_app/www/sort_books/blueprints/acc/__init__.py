...
def profile(user_id):
	...
	if not resp==None:
		...
		if not pref[3] and not 'auth' in session:
			...
			count = cur.fetchall()
			info = {'name': resp[1], 'book_count': len(count), 'book_title': '', 'email': 'ukryto', 'address': 'ukryto', 'owner': False}
	...
	if request.method == 'POST':
		if 'pass' in request.form:
			...
			if sha512(request.form['old_pass'].encode('UTF-8')).hexdigest() == cur.fetchone()[0] and request.form['new_pass']==request.form['new_pass2']:
				cur.execute("UPDATE librarians.readers SET pass=%s WHERE id_r = %s;", (sha512(request.form['new_pass'].encode('UTF-8')).hexdigest(), str(session['auth']['id'])))
