...
def book_info(id_book):
	...
	if request.method == 'GET':
		...
		cur.execute("INSERT INTO librarians.comments VALUES ("+str(len(cur.fetchall()))+", "+str(id_book)+", '%s', '%s', '%s');"%(request.form['comment'], datetime.datetime.now().strftime('%d-%m-%y'), session['auth']['username']))
		return redirect(url_for('book_blueprint.book_info', id_book=id_book))
